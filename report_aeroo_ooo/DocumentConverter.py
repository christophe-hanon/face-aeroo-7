...
class DocumentConverter:
    ...
    def __init__(self, host='localhost', port=DEFAULT_OPENOFFICE_PORT, ooo_restart_cmd=None):
        self.initialization(host=host, port=port, ooo_restart_cmd=ooo_restart_cmd)


    def initialization(self, host='localhost', port=DEFAULT_OPENOFFICE_PORT, ooo_restart_cmd=None):
        self._host = host
        self._port = port
    ...
    def putDocument(self, data):
        ...
        except DisposedException, exception:
            raise DocumentConversionException("Someone else is using DocumentConverter, please retry later (%s)" % exception)
    ...
    def _restart_ooo(self):
        ...
        try:
            ...
            if retcode == 0:
                ...
                self.initialization(host=self._host, port=self._port, ooo_restart_cmd=self._ooo_restart_cmd)
            else:
